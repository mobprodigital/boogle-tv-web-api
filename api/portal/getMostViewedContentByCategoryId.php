	...
	else
	{
		...
		if(mysqli_num_rows($portalCheck_rs) > 0)
		{
			...
			if($portalrow = mysqli_fetch_assoc($portalCheck_rs))
			{
				...
				{
					//echo "Get all most viewed Content From All Categories and content type id";
					$data = getMostViewedContent($start,$count,$link,$portalid,$contentType,$videoBaseURL,$imageBaseURL);
				...
				else
				{
					...
					if($value)
					{
						...
						// echo "Get most Viewed Content By Category ID and content type id";
						$data = getMostViewedContentByCategoryID($req_data['categoryId'],$start,$count,$link,$portalid,$contentType,$videoBaseURL,$imageBaseURL);
						wh_log("Content Array : ".str_replace("\n"," ", print_r($data, true)));
						usort($data, 'sortByView');
