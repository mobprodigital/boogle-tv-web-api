...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	else
	{
		...
		if($portalid)
		{
			...
			// Get Content
			$getData = "SELECT * FROM $dataTable where find_in_set($portalid,`portal_ids`) and content_type ='".$carr['type']."' and status = 1 ORDER BY `insertion_time` desc limit $start,$count";
