...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	{ 
		...
		wh_log("clientID - ".$clientId." | Title - ".$title." | Language - ".$language." | Description -".$description." | city -".$city." | country -".$country." | author -".$author." | newsTime -".$postTime." | portalId - ".$portalId." | categoryId - ".$categoryId." | tags -".$tags);
		...
		else
		{
			...
			{  
				...
				//echo "Insert case";
				$insert_data = "insert into news_metadata (`cat_id`,`client_id`,`portal_ids`,`title`,`content_type`,`status`,`post_time`,`insertion_time`,`language`,`description`,`tags`,`country`,`city`,`author`) values ('$categoryId',
				'$clientId','$portalId','$title','text','1', '$postTime',NOW(),'$language','$description','$tags','$country','$city','$author')";
