			...
			else
			{
				...
				if(mysqli_num_rows($check_query_rs) > 0)
				{
					...
					if($row  = mysqli_fetch_assoc($check_query_rs))
					{
						 ...
						 $insert_logs = "Insert into logs (name,email,mobile,role,'type',created_on) values ('$name','$email',$mobile,$role,'user','$create_time')";
						 $insert_logs_rs = mysqli_query($link,$insert_logs);
						 //print_r($row);
					}
					// Ends

					$query = "delete from users WHERE uid = $id and role = $roleId and client_id = $login_clientid and reports_to = $login_uid and status = 1";
