...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	$textId = mysqli_real_escape_string($link,isset($req_data['textId'])) ? mysqli_real_escape_string($link,trim($req_data['textId'])) :'';
	...
	$response['message']="textId Parameter Missing.";
	...
	$text_data = array();
	$response['status']=false;
	$response['message']="Allowed only numbers in textId Parameter";
	$response['data']= $text_data;
	}
	...
	else
	{
		...
		if(mysqli_num_rows($portalCheck_rs) > 0)
		{
			...
			//Ends
			// Get Content
			$data = getTextByID($portalid,$textId,$link,$imageBaseURL);
